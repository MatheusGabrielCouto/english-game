...
export default function KnowledgeMapRoute() {
  ...
  return (
    <ScreenContainer scrollable>
      ...
      <VaultScreenBody hubActive="map" hubLinkMode="tab" helpText={GRAPH_UI.intro} showHelp={isEmpty} helpDefaultOpen={isEmpty}>
        {loading || !snapshot ? (
          <ScreenSkeleton variant="map" className="pb-0" />
        ) : isEmpty ? (
          <VaultEmptyState
            emoji="🧠"
            title={GRAPH_UI.emptyTitle}
            body={GRAPH_UI.emptyBody}
            ctaLabel={GRAPH_UI.emptyCta}
            onCta={() => router.push(routes.vault.library)}
          />
        ) : (
