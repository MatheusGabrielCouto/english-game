...
import { buildBackupFileName } from '../utils/backup-checksum'
import { computeBackupFileChecksum } from '../utils/backup-payload'
import { snapshotBackupPreferences } from './backup-preferences-service'
...
const countTotalRows = (
  tables: BackupTableSnapshot,
  preferences: GameBackupFile['preferences'],
): number => {
  const tableRows = BACKUP_TABLE_NAMES.reduce((sum, tableName) => sum + tables[tableName].length, 0)

  return tableRows + preferences.menuHubPinnedIds.length
}

export const buildGameBackupFile = async (params: {
  appVersion: string
  platform: string
  exportedAt?: string
}): Promise<GameBackupFile> => {
  const tables = snapshotAllTables()
  const preferences = await snapshotBackupPreferences()
  const exportedAt = params.exportedAt ?? new Date().toISOString()
  const totalRows = countTotalRows(tables, preferences)
  ...
  return { meta, tables, preferences }
}
