...
import { COACH_MARK_UI, type CoachMarkStep } from '../constants/coach-mark-steps'
import type { CoachMarkTargetRect } from '../store/coach-mark-store'

const HOLE_PADDING = 10

type CoachMarkOverlayProps = {
  step: CoachMarkStep
  rect: CoachMarkTargetRect | undefined
  stepIndex: number
  totalSteps: number
  isLastStep: boolean
  onNext: () => void
  ...
  return (
    <Modal visible transparent animationType="fade" statusBarTranslucent onRequestClose={onSkip}>
      <View className="flex-1" accessibilityViewIsModal>
        ...
        <View
          className="absolute left-4 right-4 rounded-2xl border border-primary/40 bg-surface p-5"
          style={
            tooltipOnTop
          ...
          {!hole ? (
            <Text className="mt-2 text-xs text-muted">{COACH_MARK_UI.waiting}</Text>
          ) : null}

          <View className="mt-5 gap-3">
            <Button
              label={isLastStep ? COACH_MARK_UI.finish : COACH_MARK_UI.next}
              onPress={onNext}
              disabled={!hole}
            />
            <Button label={COACH_MARK_UI.skip} variant="ghost" onPress={onSkip} />
          </View>
        </View>
      </View>
    </Modal>
