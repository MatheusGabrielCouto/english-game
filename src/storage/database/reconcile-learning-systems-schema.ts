...
export const reconcileLearningSystemsSchema = (sqlite: SQLiteDatabase): void => {
  if (!tableExists(sqlite, 'flash_decks')) {
    sqlite.execSync(`
  ...
  if (!tableExists(sqlite, 'flash_cards')) {
    sqlite.execSync(`
  ...
  if (!tableExists(sqlite, 'flash_review_log')) {
    sqlite.execSync(`
  ...
  if (!tableExists(sqlite, 'flash_study_sessions')) {
    sqlite.execSync(`
  ...
  if (!tableExists(sqlite, 'duel_player_profile')) {
    sqlite.execSync(`
  ...
  if (!tableExists(sqlite, 'duel_sessions')) {
    sqlite.execSync(`
  ...
  if (!tableExists(sqlite, 'duel_session_questions')) {
    sqlite.execSync(`
  ...
  if (!tableExists(sqlite, 'lemma_competence')) {
    sqlite.execSync(`
  ...
  sqlite.execSync(`
    INSERT OR IGNORE INTO flash_decks (
      id, name, description, cover_emoji, sort_order, new_per_day, review_cap, created_at
    ) VALUES (
      'deck_default', 'Minhas palavras', 'Palavras que você quer lembrar', '📒',
      0, 10, 80, datetime('now')
    )
  `);

  sqlite.execSync(`
    INSERT OR IGNORE INTO duel_player_profile (
      id, current_patent, patent_xp, highest_patent, stamina, stamina_updated_at,
      focus_charges, daily_duel_count, daily_reset_date
    ) VALUES (
      1, 'tourist', 0, 'tourist', 5, datetime('now'), 1, 0, date('now')
    )
  `);
};
